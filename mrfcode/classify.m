...
% Read the image
im=double(imread('myslice.png'));

% Read the training data
tr=imread('training.png');
...
subplot(1,2,2);
imagesc(tr); axis image; colormap jet;
...
% Plot the histogram of the image (ignore the zeros)
figure(1);
hist(im(find((im > 0))),32);
hold on; 
plot(20000 * normpdf([0:255],mu(1),sd(1)), 'r');
plot(20000 * normpdf([0:255],mu(2),sd(2)), 'y');
plot(20000 * normpdf([0:255],mu(3),sd(3)), 'g');
hold off;
...
hist(X, 32);
hold on; 
plot(5 * length(X) * al(1) * normpdf([0:255],mu(1),sg(1)), 'r');
plot(5 * length(X) * al(2) * normpdf([0:255],mu(2),sg(2)), 'y');
plot(5 * length(X) * al(3) * normpdf([0:255],mu(3),sg(3)), 'g');
hold off;
